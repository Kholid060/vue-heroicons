<template>
  <div id="app" class="bg-gray-100">
    <div class="bg-blue-500 justify-center align-center relative" style="min-height: 300px">
      <a href="https://github.com/Kholid060/vue-heroicons" target="_blank" class="github-btn flex items-center absolute right-0 mt-4 rounded px-3 py-2 mr-6">
    ...
    <div class="lg:w-7/12 my-6 mx-auto px-6">
      ...
      <div class="icons flex flex-wrap">
        <div v-for="icon in filteredBySearch" class="p-2 sm:w-3/12  lg:w-2/12" :title="icon.name" :key="icon.icon">
          <div class="bg-white relative h-full rounded py-4 icon-container px-2 text-center cursor-pointer" :class="[icon.selected ? 'text-blue-600 active' : 'text-gray-700']" @click="icons[icon.index].selected = !icons[icon.index].selected">
...
<script>
import MyInput from './components/website/input';
import Selected from './components/website/selected';
